// 박민우 팀장 텔레그램 챗봇
// 카카오톡 3,788개 발화 + 인비절라인 공식 교육자료 학습 기반 (공식 Anthropic SDK 적용 버전)
...
async function askClaude(userId, userMessage) {
  ...
  try {
    ...
    const reply = response.content?.[0]?.text || "죄송합니다, 답변 생성에 실패했습니다ㅠ";
    conversations[userId].push({ role: "assistant", content: reply });
    return reply;

  } catch (err) {
    console.error("Claude API 호출 중 오류 발생:", err.message);
    if (err.status) {
      console.error(`에러 상태 코드: ${err.status}, 세부 내용:`, err.error);
    }
